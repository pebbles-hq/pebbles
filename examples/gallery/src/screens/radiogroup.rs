...
fn basic() -> impl IntoWidget {
    ...
    doc("Basic")
        .description("A vertical group; picking an option selects it and reports the index.")
        .body(
            column(children![
...
fn sizes() -> impl IntoWidget {
    doc("Sizes")
        .description("Scale the whole group with .size().")
        .body(
            row(children![
...
fn colors() -> impl IntoWidget {
    doc("Colors")
        .description("Tint the selected dot with .color().")
        .body(
            row(children![
...
fn disabled() -> impl IntoWidget {
    doc("Disabled")
        ...
        .body(
            radio_group(["Enabled looks like this", "But you can't pick me"])
                .value(0)
                .disabled(true),
        )
}
