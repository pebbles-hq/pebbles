...
fn two() -> impl IntoWidget {
    doc(
        "Two panels",
        "Drag the handle to resize. Each panel has a minimum size it can't cross.",
        region(
            190.0,
...
fn three() -> impl IntoWidget {
    doc(
        "Three panels",
        "Any number of panels, each handle resizing its two neighbors.",
        region(
            190.0,
            resizable(vec![
...
fn nested() -> impl IntoWidget {
    ...
    doc(
        "Nested",
        "Panels can contain other resizable groups — an IDE-style layout.",
        region(
            190.0,
