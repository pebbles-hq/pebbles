...
fn values() -> impl IntoWidget {
    doc("Values")
        .description("A fraction in 0–1 fills the track. Here 25%, 50% and 80%.")
        .body(
            column(children![
                progress(0.25, 340.0),
                progress(0.5, 340.0),
    ...
    doc("Driven by a slider")
        ...
        .body(
            column(children![
                slider(340.0)
                    .value(40.0)
                    .on_changed(move |v| p.set(v[0] / 100.0)),
                gap_h(14.0),
...
fn indeterminate_bar() -> impl IntoWidget {
    doc("Indeterminate")
        .description("For unknown durations, .indeterminate() sweeps a segment across the track continuously.")
        .body(
        column(children![
