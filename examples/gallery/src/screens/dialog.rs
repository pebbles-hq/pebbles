...
fn form(status: Signal<String>) -> impl IntoWidget {
    doc("With a form")
        .description("Inputs work inside a dialog — type, focus with Tab, and submit.")
        .body(
            button("Edit profile")
...
fn sized(status: Signal<String>) -> impl IntoWidget {
    doc("Custom size")
        .description("Size the window with .size(w, h) and set its title.")
        .body(
            button("Open large")
                .variant(ButtonVariant::Secondary)
                .on_pressed(move || {
                    ...
                    let content = panel(
                        ...
                        body(
                            "Great for editors, inspectors and detachable panels (hello, Gravel).",
                        ),
