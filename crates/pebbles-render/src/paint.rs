...
//!   fragment, or the window's frame). One of two backends supplies it, chosen by
//!   a Cargo feature on this crate:
//!     * `vello` (default) — `Scene` IS a `vello::Scene`; the `Painter` verbs
//!       forward straight to it. GPU-compute raster; the opt-in for heavy vector.
//!     * `vello-hybrid` — `Scene` is a retained, backend-neutral **op-list**; the
//!       `Painter` verbs record ops (owning their `kurbo`/`peniko` inputs), and the
//!       shell flushes the list into a `vello_hybrid::Scene` on the GPU (Phase 4).
//!       Low-power hybrid CPU+GPU raster; the battery-first default candidate.
//! - [`Painter`] wraps `&mut Scene` and exposes the drawing verbs (fill, stroke,
//!   layers, images, glyph runs). Its method *names and signatures are identical
//!   across both backends* on purpose, so the widget layer never changes.
//!
//! The geometry/brush vocabulary ([`Affine`], [`Fill`], [`Stroke`], [`Brush`], …) is
//! `kurbo`/`peniko` — renderer-neutral by nature (both backends consume the exact
//! same paths and brushes), so it's re-exported here and imported by RenderObjects
//! from `crate::paint` rather than from any backend crate.
//!
...
pub use backend::{scene, Glyph, NormalizedCoord, Painter, Scene};

// ===========================================================================
// Backend A — full Vello (GPU compute). The Painter verbs forward to vello::Scene.
// ===========================================================================
#[cfg(feature = "vello")]
mod backend {
...
// ===========================================================================
// Backend B — Vello Hybrid (low-power). The Painter records verbs into a retained,
// backend-neutral op-list; the shell flushes it to a `vello_hybrid::Scene` (Phase 4).
// ===========================================================================
