...
### Changed — bundled icon set: Lucide → Tabler (outline + filled)
The default icon set is now **Tabler Icons** (MIT), replacing Lucide. Tabler is more
complete and ships **two styles**: ~5100 **outline** glyphs at the `tabler` module root
(`tabler::CAMERA`) and ~1050 **filled** solids under `tabler::filled`
(`tabler::filled::CAMERA`) — the same `IconData` model (both 24-unit viewbox, stroke 2 /
solid). `IconKind`'s named handles now resolve to Tabler glyphs. Generated by the new
`scripts/gen-tabler.mjs` from `@tabler/icons`' node JSON; the old `lucide` module and
`gen-lucide.mjs` are removed. **Migration:** `lucide::X` → `tabler::X` (most names are
identical; a handful renamed, e.g. `TRIANGLE_ALERT`→`ALERT_TRIANGLE`,
`CHEVRONS_UP_DOWN`→`SELECTOR`, `INFO`→`INFO_CIRCLE`, `DOLLAR_SIGN`→`CURRENCY_DOLLAR`).
NOTICE updated to the Tabler MIT license. Every generated path is validated against
kurbo's SVG parser (`all_tabler_paths_parse`).
